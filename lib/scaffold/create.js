...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'berycore-lib': '^' + bitcore.version,
    'berycore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/berycore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the berycore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the berycore-node directory and configuration
